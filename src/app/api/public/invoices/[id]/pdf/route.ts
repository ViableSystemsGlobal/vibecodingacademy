...
export async function GET(
  ...
  try {
    ...
    // Get invoice - no authentication required for public access
    ...
    const issueDate = new Date(invoiceData.issueDate || invoiceData.createdAt).toLocaleDateString();
    ...
    const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || process.env.NEXTAUTH_URL || 'http://localhost:3000';
    const publicPdfUrl = `${baseUrl}/api/public/invoices/${invoice.id}/pdf`;
    let qrCodeDataUrl = '';
    try {
    ...
    // Check for discounts
    const hasDiscounts = invoiceData.lines?.some((line: any) => line.discount > 0) || false;
        ...
        <div class="content">
          <div class="header-section">
            ...
            <div class="document-info">
              ...
              <div class="document-date">Issue Date: ${issueDate} | Due Date: ${dueDate}</div>
            </div>
          </div>
          ...
          <div class="info-grid">
            <div>
              ...
              <div class="info-value">${invoice.owner.name}</div>
              <div class="info-sub">${invoice.owner.email}</div>
            </div>
            <div>
            ...
            <div>
              ...
              <div class="info-value">
                <span class="status-badge payment-status-${(invoiceData.paymentStatus as any).toLowerCase()}">${(invoiceData.paymentStatus as any).toLowerCase().replace('_', ' ')}</span>
              </div>
              <div class="info-sub">
                <div>Amount Due: ${formatCurrency(Number(invoiceData.amountDue as any))}</div>
                ${invoiceData.amountPaid > 0 ? `<div>Amount Paid: ${formatCurrency(invoiceData.amountPaid)}</div>` : ''}
              </div>
            </div>
          </div>
          ...
          ${qrCodeDataUrl ? `
            <div class="qr-code-section">
              ...
              <div class="qr-code-text">Scan to view invoice details</div>
            </div>
          ` : ''}
        </div>
    ...
    console.error('❌ Public Invoice PDF API Error:', error);
