...
interface EditProjectResourceRequestModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export function EditProjectResourceRequestModal({
  isOpen,
  onClose,
  onRequestUpdated,
  projectId,
  ...
  existingTasks = [],
  existingIncidents = [],
}: EditProjectResourceRequestModalProps) {
  ...
  useEffect(() => {
    if (isOpen && resourceRequest) {
      // Populate form with existing data
      const neededByDate = resourceRequest.neededBy 
        ? new Date(resourceRequest.neededBy).toISOString().slice(0, 16)
        : "";

      setFormData({
        title: resourceRequest.title || "",
        details: resourceRequest.details || "",
        neededBy: neededByDate,
        priority: resourceRequest.priority || "NORMAL",
        taskId: resourceRequest.taskId || "",
        incidentId: resourceRequest.incidentId || "",
        stageId: resourceRequest.stageId || "",
        emailTo: resourceRequest.emailTo || "",
        emailCc: resourceRequest.emailCc || "",
      });

      // Populate items
      if (resourceRequest.items && resourceRequest.items.length > 0) {
        setItems(
          resourceRequest.items.map(item => ({
  ...
  }, [isOpen, resourceRequest, projectId]);
  ...
  const handleClose = () => {
    if (resourceRequest) {
      ...
      setFormData({
        ...
        incidentId: resourceRequest.incidentId || "",
        stageId: resourceRequest.stageId || "",
        emailTo: (resourceRequest as any).emailTo || "",
        emailCc: (resourceRequest as any).emailCc || "",
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const errorMessage = errorData?.error || errorData?.details || "Failed to update resource request";
      ...
      success("Resource request updated successfully");
      if (onRequestUpdated) {
      ...
      const errorMessage = error.message || "Failed to update resource request";
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Edit Resource Request</DialogTitle>
          <DialogDescription>
