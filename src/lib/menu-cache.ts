...
function reconstructMenuItems(
  ...
  return cachedItems.map((cachedItem) => {
    ...
    if (cachedItem.children && originalItem.children) {
      ...
      }).filter((child): child is ModuleNavigationItem['children'][0] => child !== null);
    }
...
async function getEnabledModuleSlugs(): Promise<Set<string> | null> {
  try {
    ...
    const enabledSlugs = new Set(modules.map((m: { slug: string }) => m.slug));
...
function filterNavigationItems(
  ...
  return items
    .filter((item) => {
      // Check if module is enabled (only filter if we have enabled modules data)
      // If enabledModuleSlugs is null, it means modules haven't been synced yet - show all
      if (enabledModuleSlugs !== null && enabledModuleSlugs.size > 0) {
        // Map navigation module name to actual module slug
        const moduleSlug = getModuleSlugForNavigationModule(item.module);
        if (!enabledModuleSlugs.has(moduleSlug)) {
          console.log(`⚠️ Filtering out ${item.name} - module ${item.module} (slug: ${moduleSlug}) not enabled`);
          return false;
        }
      }
      ...
      if (!hasAccess) {
        console.log(`⚠️ Filtering out ${item.name} - user doesn't have access to module ${item.module}`);
        return false;
      }

      // Filter children if they exist
      // Children inherit the parent module's enabled status, so we only check permissions
      if (item.children && item.children.length > 0) {
        const filteredChildren = item.children.filter((child) => {
          ...
          // Default access control - only check permissions, not module enable status
          // Children inherit enabled status from parent module
        ...
        if (filteredChildren.length === 0 && !item.href) {
          console.log(`⚠️ Filtering out ${item.name} - no accessible children and no href`);
          return false;
        }
